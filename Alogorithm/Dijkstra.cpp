...
vector<pair<int,int> > g[1000];//neighbour and edge select kortesi 
...
void dijkstra(int source,int node)
{
    ...
    while(!q.empty())
    {
        ...
        int parent=u.second;
        //s.insert(parent);
        for(auto v:g[parent]){//v mane holo 0 theke suru ar g[parent] er size prjonto eita print hobe
            int child=v.first;
            int weight=v.second;
            relax(parent,child,weight);
        }
    //     for(int i = 0; i < g[parent].size(); i++){//node theke kon kon jaigai jawa jai
    // int child = g[parent][i].first;
    // int weight = g[parent][i].second;
    // relax(parent, child, weight);
}
    }
